...
var day24Part2 = function () {
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    $.each(inputStrs, (idx, str) => {
      ...
      for (var s = 5; s < strarr.length; s++) {
        if (strarr[s].includes('weak')) {
          ...
          s+=3
          group.attack = Number(strarr[s]) + (immuneGrp ? boost : 0)
          group.attackType = strarr[++s]
        } else if (strarr[s] === 'initiative') {
          group.initiative = Number(strarr[++s])
        }
    ...
    var infectionCount = allGroups.length - immuneCount
    var timeout = 10000
    while (immuneCount !== 0 && infectionCount !== 0 && --timeout) {
      ...
      while (attackers.length > 0) {
        ...
        if (targets.length > 0) {
          ...
          finalTarget = targets[0] // TODO: store this somewhere
          if (finalTarget.dmg > 0) { // don't add attacks that don't do damage
            var defIdx = defenders.findIndex((x)=>{
              return x.id === finalTarget.id
            })
            defenders.splice(defIdx,1) // only this group can attack this target in this turn
      ...
      while (attackers.length > 0) {
        var next = attackers.shift()
        // if attacking group is dead, skip
        if (allGroups.find((x)=>{
            return x.id === next.id
          }) === undefined) {
          continue
        ...
        var targetId = pair[1]
        // if defending group is dead, skip
        if (allGroups.find((x)=>{
      ...
      immuneCount = allGroups.reduce((acc,val)=>{
        return acc + (val.type==='immune' ? 1 : 0)
      },0)
      infectionCount = allGroups.length - immuneCount
      // console.log(allGroups)
    } // end round
    if (!timeout) {
      // timeout needed in case it remains an immune atkdef loop
      console.log('timeout!')
    }
    ...
    // console.log(allGroups)
    var immuneWin = immuneCount > 0
    var infectionDead = infectionCount === 0
    // console.log(
    //   'immune wins?',immuneWin,
    //   'infections dead?',infectionDead,
    //   'remaining units',totalRemainingUnits,
    //   'boost',boost)
    // 4428 CORRECT
    // 4430 too high
